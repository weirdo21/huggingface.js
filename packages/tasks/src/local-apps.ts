...
export type LocalApp = {
  ...
  | {
      ...
      snippet: (
        model: ModelData,
        filepath?: string
      ) => string | string[] | LocalAppSnippet | LocalAppSnippet[];
    }
);
...
function isAmdRyzenModel(model: ModelData) {
  return (
    model.tags.includes("ryzenai-hybrid") || model.tags.includes("ryzenai-npu")
  );
}
...
const snippetDockerModelRunner = (
  model: ModelData,
  filepath?: string
): string => {
...
const snippetLemonade = (
  ...
  if (
    model.tags.some((tag) => ["ryzenai-npu", "ryzenai-hybrid"].includes(tag))
  ) {
...
export const LOCAL_APPS = {
  ...
  "mlx-lm": {
    ...
    displayOnModelPage: (model) =>
      model.pipeline_tag === "text-generation" && isMlxModel(model),
  ...
  lmstudio: {
    ...
    deeplink: (model, filepath) =>
      new URL(
        `lmstudio://open_from_hf?model=${model.id}${
  ...
  jellybox: {
    ...
    deeplink: (model) => {
      if (isLlamaCppGgufModel(model)) {
        ...
        return new URL(
          `jellybox://image/models/huggingface/ImageLora/${model.id}`
        );
      } else {
  ...
  recursechat: {
    ...
    displayOnModelPage: isLlamaCppGgufModel,
    deeplink: (model) =>
      new URL(`recursechat://new-hf-gguf-model?hf-model-id=${model.id}`),
  },
  drawthings: {
    ...
    deeplink: (model) => {
      if (model.tags.includes("lora")) {
        return new URL(
          `https://drawthings.ai/import/diffusers/pipeline.load_lora_weights?repo_id=${model.id}`
        );
      } else {
        return new URL(
          `https://drawthings.ai/import/diffusers/pipeline.from_pretrained?repo_id=${model.id}`
        );
      }
    },
  },
  diffusionbee: {
    ...
    deeplink: (model) =>
      new URL(
        `https://diffusionbee.com/huggingface_import?model_id=${model.id}`
      ),
  },
  joyfusion: {
    ...
    macOSOnly: true,
    displayOnModelPage: (model) =>
      model.tags.includes("coreml") &&
      model.tags.includes("joyfusion") &&
      model.pipeline_tag === "text-to-image",
    deeplink: (model) =>
      new URL(`https://joyfusion.app/import_from_hf?repo_id=${model.id}`),
  },
  invoke: {
    prettyLabel: "Invoke",
    ...
    displayOnModelPage: (model) =>
      ...
      model.pipeline_tag === "text-to-image",
    deeplink: (model) =>
      new URL(`https://models.invoke.ai/huggingface/${model.id}`),
  },
  ollama: {
    prettyLabel: "Ollama",
  ...
  lemonade: {
    ...
    mainTask: "text-generation",
    displayOnModelPage: (model) =>
      isLlamaCppGgufModel(model) || isAmdRyzenModel(model),
